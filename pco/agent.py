...
"""),
            ...
            # --- 新しい指示を修正 (f-stringと{{}}を外し、プレースホルダーに変更) ---
            ("system", "You have access to a shared directory mounted at `{shared_dir_path}` inside the sandbox. This directory is for you to manage project files, download external code, and persist changes. You can use standard shell commands (e.g., `ls`, `cd`, `cat`, `echo`, `mkdir`, `rm`, `cp`, `mv`) within the `run_code_in_sandbox` tool to interact with files and directories in this shared space. Remember to always specify paths relative to `{shared_dir_path}` when operating within this directory."),
            ("system", "For GitHub operations like cloning repositories, you should use shell commands such as `git clone <repository_url> {shared_dir_path}/<destination_folder>` inside the `run_code_in_sandbox` tool. Please ensure that the `base_image` you select for the sandbox (e.g., `ubuntu/git` or a custom image you know contains `git`) has the `git` command installed if you intend to perform `git` operations. If `git` is not available in the chosen `base_image`, you should inform the user."),
            ("system", "When you need to examine the content of the shared directory, you can use `ls -F {shared_dir_path}`. To read a file, use `cat {shared_dir_path}/path/to/file.py`. To modify or create a file, you might use `echo 'new content' > {shared_dir_path}/path/to/file.py` or a more complex script depending on the modification."),
            # --- 新しい指示はここまで ---
        ...
        self.agent_runnable = (
            self.prompt_template.partial(
                ...
                # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↓修正開始◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
                shared_dir_path=config.SHARED_DIR_CONTAINER_PATH, # 新しいpartial引数を追加
                # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↑修正終わり◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
            )
            | self.llm
